...
beforeAll(async () => {
    ...
    await db.User.bulkCreate([
        ...
        {
            rut: PROFESSIONAL_RUT,
            nombre: 'Doctora Flores',
            correo: 'profesional@example.test',
            rol: 'doctor',
            contrasena_hash: professionalHash,
        },
        {
            rut: PATIENT_RUT,
            nombre: 'Paciente Perez',
            correo: 'paciente@example.test',
            rol: 'paciente',
            contrasena_hash: patientHash,
        },
    ]);
...
describe('patient flow system test', () => {
    it('permite gestionar el flujo completo de pacientes y profesionales', async () => {
        ...
        const adminToken = await trackOperation(
            () => loginUser(ADMIN_RUT, ADMIN_PASSWORD),
            metrics
        );
        const authHeaders = {
        ...
        const profesionalResponse = await trackOperation(
            () =>
                fetch(`${baseUrl}/profesionales`, {
                    method: 'POST',
                    headers: authHeaders,
                    body: JSON.stringify({
        ...
        const profesionalInDb = await database.Profesional.findByPk(
            profesionalIdNumber
        );
        expect(profesionalInDb?.user_id).toBe(PROFESSIONAL_RUT);

        const pacienteResponse = await trackOperation(
            () =>
                fetch(`${baseUrl}/pacientes`, {
                    ...
                    body: JSON.stringify({
                        ...
                        sexo: 'F',
                        comentarios:
                            'Paciente creada por el sistema de pruebas',
        ...
        const atencion = await database.Atencion.findOne({
            where: {
                paciente_id: pacienteIdNumber,
                profesional_id: profesionalIdNumber,
            },
        });
        expect(atencion).not.toBeNull();

        const profesionalLookup = await trackOperation(
            () =>
                fetch(`${baseUrl}/pacientes/${pacienteIdNumber}/profesional`, {
                    method: 'GET',
        ...
        const patientToken = await trackOperation(
            () => loginUser(PATIENT_RUT, PATIENT_PASSWORD),
            metrics
        );
        const meResponse = await trackOperation(
            () =>
                fetch(`${baseUrl}/pacientes/me`, {
                    method: 'GET',
        ...
        metrics.end();
        metrics.printReport('Patient Flow Test');

        // Assertions sobre las métricas
        expect(metrics.getErrorRate()).toBeLessThan(1); // Menos de 1% de errores
        expect(metrics.getP95()).toBeLessThan(2000); // P95 < 2s
    ...
    return await new Promise((resolve, reject) => {
        ...
        server.listen(0, () => {
            ...
            if (!address || typeof address === 'string') {
                server.close(() =>
                    reject(new Error('No se pudo determinar un puerto libre.'))
                );
                return;
            }
