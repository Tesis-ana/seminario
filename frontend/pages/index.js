...
export default function Home() {
  ...
  const redirectByRole = (tok) => {
    try {
      ...
      if (rol === 'doctor' || rol === 'enfermera') router.push('/profesional');
  ...
  useEffect(() => {
    ...
    if (stored) { setToken(stored); redirectByRole(stored); }
  }, []);
      ...
      <Layout subtitle="Inicio de sesion">
        <div className="card" style={{maxWidth:480}}>
          ...
          <form onSubmit={handleSubmit} className="stack">
            ...
            <input type="password" placeholder="Contrasena" value={contra} onChange={(e) => setContra(e.target.value)} />
