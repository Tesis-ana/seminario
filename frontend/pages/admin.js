...
import { formatRUT, validateRUT, RUTInput } from '../lib/rut';
...
export default function Admin() {
    ...
    const handleUserSubmit = async (e) => {
        ...
        if (!validateRUT(userForm.rut)) {
            alert('RUT inválido. Por favor verifique el formato.');
            return;
        }
    ...
    const handleProfSubmit = async (e) => {
        ...
        // Validar RUT del usuario antes de enviar
        if (profForm.user_id && !validateRUT(profForm.user_id)) {
            alert('RUT de usuario inválido. Por favor verifique el formato.');
            return;
        }
    ...
    const segRate = imagenes.length
        ? Math.round((segs.length / imagenes.length) * 100)
        : 0;
    const altoRiesgo = pwats.reduce((acc, p) => {
        const sum = [1, 2, 3, 4, 5, 6, 7, 8].reduce(
            (s, i) => s + (p[`cat${i}`] || 0),
            0
        );
        return acc + (sum >= 16 ? 1 : 0);
    }, 0);
    const dist = pwats.reduce(
        (d, p) => {
            const sum = [1, 2, 3, 4, 5, 6, 7, 8].reduce(
                (s, i) => s + (p[`cat${i}`] || 0),
                0
            ...
            <div className='grid stats'>
                <div className='stat-card'>
                    ...
                    <div className='stat-content'>
                        ...
                        <div className='stat-value'>{pacientesActivos}</div>
                    </div>
                </div>
                <div className='stat-card'>
                    <div
                    ...
                    <div className='stat-content'>
                        <div className='stat-title'>Casos alto riesgo</div>
                        <div className='stat-value'>{altoRiesgo}</div>
                        <div className='stat-note'>critico</div>
                    </div>
                </div>
                <div className='stat-card'>
                    <div
                    ...
                    <div className='stat-content'>
                        <div className='stat-title'>Tasa de segmentacion</div>
                        <div className='stat-value'>{segRate}%</div>
                    </div>
                </div>
                <div className='stat-card'>
                    <div
                    ...
                    <div className='stat-content'>
                        <div className='stat-title'>Analisis realizados</div>
                        <div className='stat-value'>{analisisRealizados}</div>
                    </div>
                </div>
            </div>
                    ...
                    <div className='card'>
                        ...
                        <form onSubmit={handleProfSubmit}>
                            ...
                            <RUTInput
                                ...
                                placeholder='User RUT (ej: 12.345.678-9)'
                            />
                            <input
                                value={profForm.fecha_ingreso}
                                onChange={(e) =>
                                    setProfForm({
                                        ...profForm,
                                    ...
                                    {segs.map((s) => (
                                        <tr key={s.id}>
                                            ...
                                            <td>
                                                {s.imagen_id && (
                                                    <img
                                                        src={`${BACKEND_URL}/segmentaciones/${s.imagen_id}/mask`}
                                                        alt='mask'
                                                        width={64}
                                                        height={64}
                                                    />
